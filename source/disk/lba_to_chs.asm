...
; [Disk] Переводим LBA адрес в CHS адрес (% - остаток от деления)
; Параметры:
;   - ax - LBA
; Вывод:
;   - cx [bits 0-5] - сектор
...
lba_to_chs:
    ...
    mov cx, dx                       ; Перемещаем в нужный регистр (cx - сектор)
    ...
    mov ch, al                       ; Перемещаем в нужный регистр (ch = циллиндр (первые 8 bits)
